...
- `wsjrdp_ledger_accounts`: + `additional_info` jsonb, `account_kind` NOT NULL,
...
- `wsjrdp_personal_accounts`: `default_account/cost_center/sphere` →
  `*_number` suffix, + `account_kind` NOT NULL default `'CREDITOR'` (+ CHECK),
...
6. **Naming convention drift**: bookings consistently use `*_number`
   ...
   schema touch.
7. **`account_kind` duplication and nullability**: bookings carry
   `account_kind`/`offsetting_account_kind` `NOT NULL`; the source column
   `wsjrdp_ledger_accounts.account_kind` is nullable (currently 0 NULLs). Both
   are derived from the same number heuristic
   (`account_kind_for_account_number`), not joined — they can only drift if
...
financial year ≥ 2026 it equalled `sphere_number` exactly (in SKR42, KOST1 *is*
...
**Write-back:** the link + provenance live on the entry now
(`accounting_entries.datev_booking_id`, 1:1 unique — see
`20260829130000_reverse_datev_booking_links`). The booking has no own person;
its person is `entry.subject`.

- **Tier 1 (2026, deterministic)** is implemented as `Fin::DatevBookingMatcher`
  ...
  **of the person named in the Buchungstext** (extra safety check on top of
  the pn id). Matchable rows are marked in the booking's detail row
  (the "Verknüpfungen" block of `fin/bookings/_booking_detail`).
...
**Matcher v2 (2026-08-20, review UI).** `Fin::DatevBookingMatcher.propose` returns
...
handling later. `connect!` sets the entry's `datev_booking_id` + provenance in
ONE bulk UPDATE and mirrors the entry's camt onto `wsjrdp_camt_transactions.datev_booking_id`
...
Auswahl / Seite / Alle connect modes. The importer mirrors the camt link the
same way after its 2025 matching. All links were
...
(`GET /fin/bookkeeping/bookings/:id/query_entries`). On "moving the fuzzy matching to SQL": PostgreSQL could do it
...
Three changes to `Fin::DatevBookingMatcher` / the reconciliation UI:
...
`account_kind`/`offsetting_account_kind`.
...
`Wsjrdp::Filtering::Options.merge` now prefers the first **informative** label and
orders by number.

## 4. Polymorphic association — Option 1 IMPLEMENTED

> **Status:** Option 1 is implemented (2026-08-20): the two STORED GENERATED
> `account_type` / `offsetting_account_type` columns (holding the polymorphic
> target class, derived from `account_kind`) live in the create migration and
> the dev DB, and
...
function of the existing `*_account_kind` (`CREDITOR` ⇔ supplier). What's
...
t.virtual :account_type, type: :string, stored: true,
  as: "CASE WHEN account_kind = 'CREDITOR' THEN 'WsjrdpPersonalAccount' ELSE 'WsjrdpLedgerAccount' END"
t.virtual :offsetting_account_type, type: :string, stored: true,
  as: "CASE WHEN offsetting_account_kind = 'CREDITOR' THEN 'WsjrdpPersonalAccount' ELSE 'WsjrdpLedgerAccount' END"
```
...
class DatevBooking < ActiveRecord::Base
  belongs_to :konto, polymorphic: true,
    foreign_key: :account_number, foreign_type: :account_type,
    primary_key: :number
  belongs_to :gegenkonto, polymorphic: true,
    foreign_key: :offsetting_account_number, foreign_type: :offsetting_account_type,
    primary_key: :number
end
```
...
- **Cannot drift**: the class name is computed by the DB from `account_kind`,
...
`account_kind` on any manual edit. Option 1 strictly dominates.
...
`account_kind`. Works today with no schema change, but `konto` is a method,
...
- **Name collision defused**: the association is named `account` while a
  column `account_kind` (the DATEV Kontenart) already exists — the explicit
  `foreign_type: :account_type` fully overrides the `#{name}_type`
  convention; `booking.account_kind` still returns the Kontenart, and
  generated SQL touches only `account_type`.
- `where(offsetting_account: supplier)` compiles to
  `offsetting_account_type = 'WsjrdpPersonalAccount' AND
...
class DatevBooking < ActiveRecord::Base
  ...
  def konto
    (account_kind == "CREDITOR") ? (konto_supplier || konto_ledger_account) : konto_ledger_account
  end

  def gegenkonto
    (offsetting_account_kind == "CREDITOR") ? (gegenkonto_supplier || gegenkonto_ledger_account) : gegenkonto_ledger_account
  end
end
```
...
```ruby
t.virtual :account_type, type: :string, stored: true,
  as: "CASE WHEN account_kind = 'CREDITOR' THEN 'supplier' ELSE 'ledger_account' END"
t.virtual :offsetting_account_type, type: :string, stored: true,
  as: "CASE WHEN offsetting_account_kind = 'CREDITOR' THEN 'supplier' ELSE 'ledger_account' END"
```
...
belongs_to :konto, polymorphic: true, optional: true,
  foreign_key: :account_number, foreign_type: :account_type, primary_key: :number
belongs_to :gegenkonto, polymorphic: true, optional: true,
  foreign_key: :offsetting_account_number, foreign_type: :offsetting_account_type, primary_key: :number
```
