...
Status: **implemented.** Parts 1–3 are the design the engine was built from and
are kept as the shared model; Part 4 records what shipped and where it deviates.
Code snippets in Parts 1–3 are illustrative sketches, not the final APIs —
`app/domain/wsjrdp/filtering/` is authoritative.

> **Note on the examples.** The engine (`Wsjrdp::Filtering::*`) is generic; the
> bookings schema `Fin::DatevBookingsFilterSchema` ships alongside it as its first
> consumer and as the fixture the engine spec runs against. The *pages* using
> it (the Buchungen controller and its views) arrive in a later PR.
...
attribute so the user cannot filter on it at all; today that is the `exclude:`
option of the table's `filter:` policy, see Part 4): a hidden *attribute* removes
a control, a hidden *condition* forces a value. The per-account detail page uses
both — it hides the account attribute **and** fixes `account = X` as a hidden
condition.
...
```
                       ┌───────────────────────────────────────────┐
 declares attributes   │  Wsjrdp::Filtering::Schema  (per model / per join) │
 ──────────────────▶   │   base relation + registered Attributes    │
                       └───────────────┬───────────────────────────┘
                                       │ catalog (JSON)         ▲ options (JSON / endpoint)
                                       ▼                        │
   ┌───────────────┐   value (JSON)  ┌────────────────┐   scope ┌───────────────────────┐
   │ Generic JS UI │ ──────────────▶ │ Wsjrdp::Filtering::Query│ ──────▶ │ Wsjrdp::Filtering::Compiler   │
...
attribute names the operators it supports **explicitly** (§2.11 #8). The
vocabulary sketched here is the v1 set; the operators added later are recorded
in Part 4 and the code holds the current list.
...
**The vocabulary today** (the code in `app/domain/wsjrdp/filtering.rb` stays the
source of truth; a type implements an operator only where the row says so):

| key | wire | arity | types | label(s) |
|---|---|---|---|---|
| `in` | `in` | many | reference, enum | "ist" / "ist eines von" |
| `not_in` | `ni` | many | reference, enum | "ist nicht" / "ist keines von" |
| `present` | `pr` | none | reference, decimal, date | "hat Wert" |
| `blank` | `bl` | none | reference, decimal, date | "ist leer" |
| `eq` | `eq` | one | decimal, text | "=" (decimal) / "ist genau" (text) |
| `gt` | `gt` | one | decimal | ">" |
| `gte` | `ge` | one | decimal, date | "≥" / "ab" |
| `lt` | `lt` | one | decimal, date | "<" / "vor" |
| `lte` | `le` | one | decimal, date | "≤" / "bis" |
| `between` | `bt` | two | decimal, date | "im Bereich" (closed both ends) |
| `nonzero` | `nz` | none | decimal | "≠ 0" |
| `in_month` | `im` | one | date | "im Monat" (operand `YYYY-MM`) |
| `in_year` | `iy` | one | date | "im Jahr" (operand year) |
| `contains` / `contains_cs` | `ct` / `ctc` | one | text | "enthält" (+ case toggle) |
| `not_contains` / `not_contains_cs` | `nct` / `ncc` | one | text | "enthält nicht" |
| `glob` / `glob_cs` | `gl` / `glc` | one | text | "Glob" (`*` / `?`, whole value) |
| `eq_cs` | `eqc` | one | text | "ist genau" (case-sensitive half of `eq`) |
| `regex` / `regex_cs` | `re` / `rec` | one | text | "Regex" |

Two notes on the comparison half. **`lte` is the inclusive upper bound** next to
the exclusive `lt` — the end `between` has always used — which is why the date
labels read "ab" / "bis" / "vor". **`nonzero` takes no operand**: it is the
"has a value at all" atom of a signed amount (`col <> 0`), and NULL stays
UNKNOWN under 3VL (§1.3) exactly as it does for every comparison, so a row
without a value is excluded either way.

Predefined, reusable types (the atom vocabulary lives here, once):

```ruby
module Wsjrdp::Filtering::Types
  REFERENCE = Wsjrdp::Filtering::Type.new(key: :reference, control: "multiselect", operators: [
    Wsjrdp::Filtering::Operator.new(key: :in,      label: "ist",       arity: :many) { |c, v| c.in(v) },
    Wsjrdp::Filtering::Operator.new(key: :not_in,  label: "ist nicht", arity: :many) { |c, v| c.not_in(v) },
    Wsjrdp::Filtering::Operator.new(key: :present, label: "hat Wert",  arity: :none) { |c, _| c.not_eq(nil) },
    Wsjrdp::Filtering::Operator.new(key: :blank,   label: "ist leer",  arity: :none) { |c, _| c.eq(nil) },
  ])

  DECIMAL = Wsjrdp::Filtering::Type.new(key: :decimal, control: "number_range", operators: [
    Wsjrdp::Filtering::Operator.new(key: :gte,     label: "≥",          arity: :one) { |c, v| c.gteq(v[0]) },
    Wsjrdp::Filtering::Operator.new(key: :lt,      label: "<",          arity: :one) { |c, v| c.lt(v[0]) },
    Wsjrdp::Filtering::Operator.new(key: :between, label: "im Bereich", arity: :two) { |c, v| c.gteq(v[0]).and(c.lteq(v[1])) },
    Wsjrdp::Filtering::Operator.new(key: :present, label: "hat Wert",   arity: :none){ |c, _| c.not_eq(nil) },
    Wsjrdp::Filtering::Operator.new(key: :blank,   label: "ist leer",   arity: :none){ |c, _| c.eq(nil) },
  ])

  # Same operator KEYS as decimal (one global concept), date-specific labels.
  DATE = Wsjrdp::Filtering::Type.new(key: :date, control: "date_range", operators: [
    Wsjrdp::Filtering::Operator.new(key: :gte,     label: "ab",         arity: :one) { |c, v| c.gteq(v[0]) },
    Wsjrdp::Filtering::Operator.new(key: :lt,      label: "vor",        arity: :one) { |c, v| c.lt(v[0]) },
    Wsjrdp::Filtering::Operator.new(key: :between, label: "im Bereich", arity: :two) { |c, v| c.gteq(v[0]).and(c.lteq(v[1])) },
    Wsjrdp::Filtering::Operator.new(key: :present, label: "hat Wert",   arity: :none){ |c, _| c.not_eq(nil) },
    Wsjrdp::Filtering::Operator.new(key: :blank,   label: "ist leer",   arity: :none){ |c, _| c.eq(nil) },
  ])
  ...
  TEXT = Wsjrdp::Filtering::Type.new(key: :text, control: "text", operators: [
    Wsjrdp::Filtering::Operator.new(key: :contains, label: "enthält", arity: :one) { |cols, v|
      Array(cols).map { |c| c.matches("%#{sanitize_like(v[0])}%") }.reduce(:or)
    },
  ])

  ENUM = Wsjrdp::Filtering::Type.new(key: :enum, control: "select", operators: [
    Wsjrdp::Filtering::Operator.new(key: :in,     label: "ist",       arity: :many) { |c, v| c.in(v) },
    Wsjrdp::Filtering::Operator.new(key: :not_in, label: "ist nicht", arity: :many) { |c, v| c.not_in(v) },
  ])
end
...
module Wsjrdp::Filtering
  class Attribute
    ...
    #          ACCEPTS, resolved against the type's implementation library.
    #          Nothing is ever derived implicitly from the type; an unknown key
    #          raises at declaration time (host-authored, so fail loud).
    # pickable: the subset of `operators:` the EDITOR offers (see below);
    #          defaults to all accepted ones. A key it does not accept raises.
    # operand_min: lower bound for the editor's numeric operand inputs (the
    #          `min` attribute) — cosmetic guidance, never re-validated server-side.
    ...
    #          first declared member is the default (the text search over
    #          Buchungstext / Belege / beide, and an amount next to its absolute
    #          value; see Part 4, UX iterations 2 and 6).
    # sign:    :signed | :absolute — the two members of an amount's SIGN PAIR
    #          (the signed column and its ABS() twin). The editor renders such a
    #          group as a ± / |x| toggle instead of the sub-variant dropdown.
    #          Schema.define checks the pairing: a variant group either declares
    #          no sign at all, or has exactly two members, one of each.
    def initialize(key:, label:, type:, column:, operators:, pickable: nil,
      short_key: key, group: nil, options: nil, catalog: true, operand_min: nil,
      sign: nil)
    ...
    def catalog?           = @catalog
    def operators          = @operators.values              # ACCEPTED
    def operator(key)      = @operators[key]   # nil if not accepted on this attribute
    def pickable_operators = operators.select { |o| pickable?(o.key) }   # OFFERED

    def as_json(*)
      { key:, label:, group:, variant_group:, sign:,
        type: type.key, control: type.control, operand_min:,
...
#### Accepted vs. pickable operators

`operators:` is what an attribute **accepts**; `pickable:` is the subset the
**editor offers** in its operator dropdown (default: all accepted ones). The
split exists because those are two different audiences:

| goes by the ACCEPTED list | goes by the PICKABLE list |
|---|---|
| URL decoding (`UrlCodec`) and encoding | the editor's operator dropdown |
| a policy's fixed slots and its `default:` tree | the operator a fresh condition starts on |
| filter **presets** | — |
| the compiler | — |
| chip labels (a chip labels anything accepted) | — |

So an operator can stay valid on the wire and in host-authored slots without
cluttering a dropdown. The finance schemas narrow nothing — every attribute
offers what it accepts, in its declared order — but the split stays, because
the two lists answer to different audiences. Two consequences worth knowing:

- **The catalog ships every accepted operator**, each flagged `pickable:
  true/false` — the UI needs the full list to *label* a condition it may not
  *offer*.
- **The edited condition's own operator is always listed**, selected, even when
  it is not pickable, so a chip that arrived from a hand-written URL or a preset
  can be edited (or just re-saved) without being silently rewritten. An
  attribute whose pickable list a `derive` narrowed to nothing falls back to the
  full list rather than leaving the dropdown empty.
...
- **Allow-list everything symbolic.** Only registered attribute keys and the
  ...
  are dropped. The same applies to `short_key`s in the URL codec. It is the same
  spirit as the table's other two allow-lists, which come from the dataset's
  column description (`Wsjrdp::ExpandableTableColumns`): its `codec` for the
  column and sort tokens in the URL, its `sort_expressions` for what may reach
  `ORDER BY`.
...
**Decision (v1): A — silent drop, with a documented loophole.**
**Superseded (2026-09): C, at RESOLUTION time — the loophole is closed.**
Host-supplied conditions are no longer a separate compiler argument at all; they
are the table policy's **fixed slots**, and the resolver parses them through
`Wsjrdp::Filtering::FilterSchema#parse_fixed!`, which **raises** on an unknown
attribute or operator, on a wrong operand count and on an operand that does not
cast — naming the attribute, the operator and the slot index. Not option B
(boot time) because the schemas bind per request (a schema may depend on the
request), and resolution is the first moment where a schema exists; the cost is
one pass over a handful of host-authored slots, and
`spec/controllers/fin/table_policies_spec.rb` resolves every declared table of
the wagon once so a typo fails in CI rather than in production.
`Compiler#apply(hidden:)` — the loophole's mechanism — is gone: the compiler
only ever sees a Query the state has already parsed (tolerantly for user input,
strictly for host input), and there is no way to hand it a raw tree.
...
- On **Anwenden**, POST the **value** JSON; the server encodes it to Rison and
  redirects (PRG) to `?f=<rison>` (§2.8). Edits before Apply mutate only the
...
| `select` | `many` | `["account_kind","in","BANK"]` | plain select for a small fixed enum (e.g. Kontenart) |
...
**Variant groups** add a second control next to the attribute select, because
one picker entry stands for several attributes (`variant_group:`): a `<select>`
of the members' labels for the text searches, and — where the members are an
amount's sign pair (`sign: :signed` / `:absolute`) — a two-button toggle, `±`
for the signed member and `|x|` for the absolute one, the edited member pressed
(`aria-pressed`). Either control switches the condition's attribute and keeps
...
- **Variant switch:** a picker entry that stands for a `variant_group` shows its
  members between the attribute select and the operator select — the dropdown
  for a text search, the `±` / `|x|` toggle for an amount's sign pair (§2.5).
...
- **Data flow (apply model, §2.8):** commits mutate only the client-side value;
  ...
  value is POSTed and the server redirects (PRG) to `?f=<rison>`. Debouncing
...
value in-place; nothing runs until the user clicks **Anwenden** — not even
removing a condition or a slot. On apply, the **URL is updated** to carry the
whole filter, the results reload, and back / forward / copy-link all work. (This
differs from Moss, which is live; we chose apply deliberately — one query per
intent, cheap sharing, no debounce races.) While the builder differs from the
applied filter, a warning chip and a badge on the filter toggle say so;
**"Änderungen verwerfen"** drops the unapplied edits client-side, and
**"Filter zurücksetzen"** applies the table's default filter (the policy's
`default:` tree, or empty).
...
`GET …?f=…` (Post/Redirect/Get). On load, the controller decodes the filter param
...
> **Param name.** The filter is field `filter` of the table's state, so its query
> param is `<prefix>f` — bare `f` for a page's only table, `bkf` for the table
> with prefix `"bk"` (see `doc/wsjrdp/expandable_table.md` §3). Never spell it
> out: `state.param_name(:filter)` gives the name for the current table.
...
|---|---|---|---|---|
| **Rison** ✅ | `(slots:!((conditions:!((attribute:amount,operator:between,operands:!(10000,25000))))))` | syntax chars are RFC-3986 query-legal unencoded; only string *data* is escaped | **Ruby `rison` gem** + JS `rison` | JSON-equivalent, compact, readable; proven in **Kibana** URL state |
...
**Decision: Rison**, via the `rison` gem (`Rison.dump` / `Rison.load`, handles
...
caveat: `rison` is a small, low-profile gem — before depending on it we should
...
in the URL:      ?f=!(!(!(amt,bt,10000,25000)),!(!(cc,in,'3150')))
```
...
- **Empty:** an absent/empty filter param = no conditions = all rows.
...
```ruby
BOOKINGS_SCHEMA = Wsjrdp::Filtering::Schema.define do |s|
  ...
  account_options = Wsjrdp::Filtering::Options.merge(
    Wsjrdp::Filtering::Options.from(WsjrdpLedgerAccount.all, value: :number, label: ->(a){ "#{a.number} #{a.name}" }),
    Wsjrdp::Filtering::Options.from(WsjrdpPersonalAccount.all,      value: :number, label: ->(s){ "#{s.number} #{s.name}" }))

  s.attribute key: :amount,             short_key: :amt, label: "Betrag",        type: Wsjrdp::Filtering::Types::DECIMAL,
              operators: %i[gte lt between],          column: :signed_base_amount
  s.attribute key: :booking_date,       short_key: :bd,  label: "Buchungsdatum", type: Wsjrdp::Filtering::Types::DATE,
  ...
  s.attribute key: :konto,              short_key: :k,   label: "Konto",         type: Wsjrdp::Filtering::Types::REFERENCE,
              operators: %i[in not_in],               column: :account_number,            options: account_options
  s.attribute key: :offsetting_account, short_key: :gk,  label: "Gegenkonto",    type: Wsjrdp::Filtering::Types::REFERENCE,
  ...
  s.attribute key: :cost_center,        short_key: :cc,  label: "Kostenstelle",  type: Wsjrdp::Filtering::Types::REFERENCE,
              operators: %i[in not_in present blank], column: :cost_center_number,
              options: Wsjrdp::Filtering::Options.from(WsjrdpCostCenter.all, value: :number, label: ->(c){ "#{c.number} #{c.name}" })
  s.attribute key: :text,               short_key: :q,   label: "Freitext",      type: Wsjrdp::Filtering::Types::TEXT,
              operators: %i[contains],
              column: ->(t) { [t[:posting_text], t[:original_posting_text], t[:document_field_1]] }
  ...
  s.attribute key: :status,             short_key: :st,  label: "Status",        type: Wsjrdp::Filtering::Types::ENUM,
...
BOOKINGS_HIDDEN = [
  Wsjrdp::Filtering::Slot.new(conditions: [
    Wsjrdp::Filtering::Condition.new(attribute: :status, operator: :in, operands: ["active"])
  ])
]
```
...
LEGS_SCHEMA = BOOKINGS_SCHEMA.derive do |s|
  ...
  s.attribute key: :leg_amount, short_key: :lam, label: "Betrag (Kontosicht)",
              type: Wsjrdp::Filtering::Types::DECIMAL, operators: %i[gte lt between],
...
query     = Wsjrdp::Filtering::UrlCodec.decode(params[:filter], schema: bound)   # Rison + short_keys -> Query
@filtered = Wsjrdp::Filtering::Compiler.new(bound)
...
### 2.10 Relationship to the rest of the table

The filter the generic engine replaced was a **restricted CNF**: it also ANDed
across fields and ORed within a field, but each OR-clause was locked to a
*single* field and the set of "slots" was fixed. The generic engine **subsumes**
it (a mixed-field OR is now possible, slots are user-defined). Everything
orthogonal — **ordering, pagination, the total count and the summary
`total_sum`** — is `Wsjrdp::ExpandableTableRows`, and the **column selection** is
the table's policy plus the dataset's `Wsjrdp::ExpandableTableColumns` module
(`doc/wsjrdp/expandable_table.md`). None of them knows about the filter: they
simply consume the relation `state.filter.scope(base)` produced, so the filter is
one step of the pipeline and nothing else had to change.
...
4. **Where the engine lives.** Wagon-local first (`app/domain/wsjrdp/filtering/…`), or
...
6. ~~**URL format.**~~ **Decided: Rison** (the `rison` gem), codec in Ruby, PRG apply
...
| `decimal` (Betrag) | `≥`, `<`, `im Bereich` | no presence — `signed_base_amount` is NOT NULL/generated |
...
- *Beträge & Daten:* **Betrag** (`signed_base_amount`), **Buchungsdatum** (`booking_date`),
...
(`primanota_period`), **Wirtschaftsjahr** (`financial_year`), **Belegfeld 1** as a
...
(`app/domain/wsjrdp/filtering/…`, since core `app/hitobito` is read-only). The generic
...
- *Max length:* cap the generated URL at ~**1800 chars** (safe under old
  proxy/browser limits). Below it, put the Rison in `?f=`; above it, fall back
  to a POST-backed apply (server holds the filter under a short token; URL carries
  a reference param). v1 filters are a few slots, so this rarely triggers —
  ship the guard, treat the token store as a later refinement.
- *Legacy params:* on the bookings index, if the filter param is absent but old params
  ...
  to the canonical `?f=…` (each multi-select → one `in` slot; each range →
...
- **`account_number` / `offsetting_account_number` / `account_kind` /
  `offsetting_account_kind` — now `NOT NULL` (decided & applied).** The raw
...
The filter is implemented. The bookings page hosting it (`/bookkeeping/bookings`,
with the previous UI frozen on `/bookkeeping/bookings_old`) arrives with the
finance pages — see the note at the top. Code layout, exactly along the
generic/specific boundary of §2.12(4):

- **Generic** (`app/domain/wsjrdp/filtering{,.rb}/`): `Operator`, `Type`, `Types`,
  `Attribute`, `Options`, `Schema`, `BoundSchema`, `Query`/`Slot`/`Condition`,
  `Compiler`, `UrlCodec` (Rison through the `rison` gem), `SlotEquality` (when
  two slots are the same, for the presets below) and `FilterSchema` (the
  protocol below) — no dataset knowledge anywhere.
  Generic UI: `app/views/shared/wsjrdp/filtering/_builder.html.haml` (self-contained
  vanilla-JS component rendered purely from catalog+value; all DOM built via
  `createElement`/`textContent`, no injection) and `app/views/shared/wsjrdp/_collapsible_panes.html.haml`
  (pane chrome / scroll restore, shared with the column picker).
- **Bookings-specific**: `app/domain/fin/datev_bookings_filter_schema.rb` (the schema
  declaration and `bound`, extending `Wsjrdp::Filtering::FilterSchema`),
  `app/domain/fin/datev_bookings_columns.rb` (the column description behind the
  policy's codec and the sort allow-list), each host controller's own
  `BOOKINGS_POLICY` and its `bookings` method — a `Wsjrdp::ExpandableTableRows`
  over `state.filter.scope(DatevBooking.all)`, which orders, pages and sums it —
  the `post :apply` route, and the `_columns` pane extracted from the old
  `_filters` partial (which is deleted; the frozen copy lives on under
  `fin/bookings_old/`).

- **The filter schema — one protocol per dataset (2026-09).**
  `Wsjrdp::Filtering::FilterSchema` is what a table's policy names as its `schema:`.
  A dataset module declares its `SCHEMA` and `bound(except:)` and `extend`s it;
  `Fin::DatevBookingsFilterSchema`, `Fin::MossTransactionsFilterSchema`,
  `Fin::PersonalAccountsFilterSchema`, `Fin::CostCentersFilterSchema` and
  `Fin::LedgerAccountsFilterSchema` all do. The
  protocol is where the tolerant and the strict path are told apart — the whole
  reason it exists:

  | method | input | strictness |
  |---|---|---|
  | `bound(except:)` → `BoundSchema` | the table's `exclude:` | — (implemented by the dataset) |
  | `decode(raw, schema:)` → `Query` | URL param / store value | **tolerant**: unknown keys dropped, garbage → empty |
  | `encode(query, schema:)` → wire \| nil | a parsed query | canonicalizes to short keys |
  | `encode_tree(tree, schema:)` → wire \| nil | the builder's posted JSON | **tolerant** (it comes from the browser) |
  | `parse_fixed!(tree, schema:)` → `Query` | host code: fixed slots, `default:` | **strict**: raises, naming attribute, operator, slot index |
  | `compile(query, schema:, relation:)` → relation | a parsed query (or nil) | merges the schema's own base relation, then `Compiler` |

  `compile` merging `schema.base` is what makes the required joins automatic: the
  bookings schema's base carries `left_joins(:batch)` (for `financial_year` /
  `batch`), the Moss one `left_joins(expenses: :bookings)`. **No host
  repeats a join**, and none can forget one.

- **Where the filter's own state lives (2026-09).** The filter is one field of
  the table's state (`doc/wsjrdp/expandable_table.md`), declared once in the host
  controller instead of through separate hooks:

  ```ruby
  wsjrdp_expandable_table_policy prefix: "bk",
    filter: {policy: :url,                                  # or :remember / :fixed
  ...
  - `schema:` names the dataset and comes **only** from this class-level
    declaration — never from a param, the store or a cookie, so no request can
    point a table at another schema. It is mandatory as soon as any other filter
    option is declared; a table that declares no `filter:` at all gets an inert
    filter (no catalog, no user query, `scope` hands its argument back).
  - `policy:` decides whether the filter lives in the URL only (`:url`, the
    default), is also remembered in the store (`:remember` — the Buchungen list
    and the Moss transactions list do that), or is entirely controller-given
    (`:fixed`, i.e. fixed slots and no user part).
  - `fixed:` replaces the former `locked_filter_tree` / `filter_hidden_conditions`
  ...
  - `default:` is the user tree shown when neither the param nor the store
    ...
    *reduced* schema, because it is the user part and must be editable in the
    builder.
  - `presets:` are the one-click toggles of the filter line above the pane
    ("Schnellauswahl"): named slot lists, or a group of buttons sharing one
    slot; see the presets note below.
  - **Resolution produces `Wsjrdp::TableState::Filter`**, and
    `state.filter.scope(base)` is the ONLY way from it to a relation: fixed slots
    compiled with the full schema, the user query with the reduced one on top.
    Its readers are `fixed_entries` / `fixed_slots` / `readonly_slots` /
    `hidden_slots` / `user_query` / `user_slots` / `effective_slots` / `exclude`
    / `catalog` / `full_catalog` / `wire` / `encode_tree` / `fixed?`. The filter
    builder partial reads all of them from the state; the views pass only the
    apply URL and display options.
  - **"Zurücksetzen" resets the FILTER only** — never the table's sort, columns,
    page size or page. It links to `et_filter_reset_url(state)`, i.e. this
    table's filter param set explicitly **blank** (`?f=` / `?bkf=`) with its page
    and open rows dropped (a filter change closes every row); every other param
    survives. Blank rather than absent, because a present-but-blank param beats
    the store while an absent one would restore the just-cleared filter on a
    remembering page — the same reason an apply that removed every condition
    redirects to a blank filter param. The wholesale table reset
    (`?<prefix>r=1`, page-wide `table_state_reset`, handled by
    `Wsjrdp::TableStateful`) still exists but has **no button in the UI**.
    A blank param means "no conditions" and beats a `default:` tree, which only
    applies on the `:default` resolution path.
  - **The apply is generic too**:
    `Wsjrdp::TableStateful#wsjrdp_apply_table_filter(state, redirect_to:)` is the
    one PRG implementation — it encodes the posted tree through
    `state.filter.encode_tree` (so the form passes the same allow-list as a URL),
    drops this table's filter, page and open rows, carries everything else, and
    always emits the filter param (blank when empty).

- **Presets — the "Schnellauswahl" (2026-09).** A preset is a named list of
  slots a table declares once (in its policy or, alternatively, in the view via
  `t.filter presets:`; both at once raises), rendered as toggle LINKS in the
  filter line above the pane. It is a shortcut *into* the user part, not a pin:
  its slots land in the `f` param and stay editable. Three properties are worth
  remembering:
  - **Exact slot equality decides whether a preset is active** — the same SET of
    conditions, order irrelevant, operands compared canonically, and an
    OR-widened slot does NOT count (it widens what the preset promised). The
    two members of a sign pair are ONE attribute under a **sign-invariant**
    operator (`≠ 0`, `hat Wert`, `ist leer`, `= 0`), so `Saldo ≠ 0` and
    `|Saldo| ≠ 0` are the same condition while a comparison stays
    member-specific. The rule lives in `Wsjrdp::Filtering::SlotEquality`, the
    pairing reaches it as `aliases:` (`Wsjrdp::Filtering::Schema#sign_aliases`,
    on the bound schema too); the builder's JS repeats both for the cosmetic
    accent stripe on a preset's slot.
  - **A toggle applies at once** (`et_url(state, {filter: preset.toggle_wire})`)
    — the one thing in the builder that does — which is exactly why every toggle
    is **locked while the builder has unapplied edits**.
  - Preset slots are host-authored and therefore parsed **strictly**
    (`parse_fixed!`, against the reduced schema).
  - **A group is the second declaration form**: `{group:, attribute:, operator:
    "in", members: [{key:, label:, value:, …}]}`, in the same list, makes
    several buttons share ONE slot `attribute in (values)` with one value each,
    so two pressed buttons widen the result where two presets would AND two
    slots and find nothing. Members are checked just as strictly — another
    operator, an attribute the reduced schema does not offer `in` on, a value
    outside its options or a repeated key raise, naming the group — and the
    shared slot gets neither a chip nor the accent stripe while every one of its
    values belongs to a member.
  The full rules, including the overlapping-preset case, are in
  `doc/wsjrdp/expandable_table.md`, "Presets".

- **A second consumer: the Kreditoren list (2026-09).**
  `Fin::PersonalAccountsFilterSchema` binds to
  `WsjrdpPersonalAccount.with_booking_summary`, whose derived table carries the
  two-sided balance and the booking count as real columns — so the same engine
  filters an aggregate as readily as a plain one. Its attributes are Name
  (TEXT over `name` **and** `short_name`), Saldo and Buchungen (DECIMAL) and
  Moss Status (ENUM over `COALESCE(moss_status, 'deactivated')`, so an account
  Moss does not know counts as inaktiv). It brought the global operator **`gt`**
  (`>`, short key `gt`) into `OPERATOR_VOCABULARY` and the decimal type, next to
  `gte` and `lt`; the bookings' `amount` offers it too. Saldo later became a
  two-member variant group, |Saldo| first — see UX iteration 6.

- **The Kostenstellen list.** `Fin::CostCentersFilterSchema` binds to
  `WsjrdpCostCenter.with_booking_summary`, the same derived-table shape, and
  declares Name (TEXT over `name` **and** `short_name`, with the glob pair),
  Moss Status (the same `COALESCE(moss_status, 'deactivated')` ENUM) and
  Buchungen (DECIMAL). Its fourth attribute, `number` (REFERENCE, `in` /
  `not_in`), is declared **`catalog: false`**: it exists only so the page can
  pin one cost center out of its list through a hidden fixed slot, never for the
  picker.

- **The Sachkonten list.** `Fin::LedgerAccountsFilterSchema` binds to
  `WsjrdpLedgerAccount.with_booking_summary`, the same derived-table shape over
  `DatevBooking.legs`, and declares Name (TEXT over `name` **and** `short_name`,
  with the glob pair), Kontoart (ENUM over the DATEV short code, labelled from
  `fin.account_kind.*`) and Moss Status (the same
  `COALESCE(moss_status, 'deactivated')` ENUM as the two other summaries). The
  page declares **no presets**: which accounts of the chart are listed at all is
  already decided by its visibility rule
  (`Fin::LedgerAccountsController#visible_ledger_accounts`), so everything above
  the list is the user's own filter.

- **Specs**: `spec/domain/wsjrdp/filtering_engine_spec.rb` (standalone, no Rails:
  vocabulary, value objects, schema define/derive/bind, URL codec, catalog),
  `spec/domain/fin/datev_bookings_filter_schema_spec.rb`,
  `spec/domain/fin/personal_accounts_filter_schema_spec.rb` and
  `spec/domain/fin/cost_centers_filter_schema_spec.rb` (Rails: the Arel
  compiler against the real schemas, and the strict `parse_fixed!` /
  `compile` half of the protocol), `spec/domain/wsjrdp/table_state_spec.rb`
  (standalone: the resolver's filter half, presets included, against a fake
  filter schema), `spec/controllers/fin/personal_accounts_controller_spec.rb`
  and `spec/controllers/fin/cost_centers_controller_spec.rb`
  (the rendered presets, their toggle URLs, the hidden fixed slot and the
  filtered footer) and
  `spec/controllers/fin/table_policies_spec.rb` (resolves every declared table of
  the wagon once). Plus a 27-check `rails runner` smoke suite run against the dev
  data during development (all green, including parity with the legacy query).

### Deviations from the plan (deliberate, flagged)

1. **Rison codec: the `rison` gem** (`rison ~> 2.1`, declared in the wagon
   gemspec) — not an in-wagon parser as an earlier version of this note said;
   there is no `Wsjrdp::Filtering::Rison`. `Wsjrdp::Filtering::UrlCodec` calls
   `Rison.dump` / `Rison.load` and rescues `Rison::ParseError`; the gem dumps
   bare symbols and parses ids back as symbols, exactly the wire form the codec
   needs. The §2.8 maintenance caveat stands: vendor the gem or write an own
   parser if it ever goes unmaintained.
...
4. **Legacy-param shim (§2.12(6)) not implemented.** Old-style URLs
   (`?account_number[]=…`) now land on the unfiltered new page; a remembered
   filter self-heals on the next apply. The frozen old page still
...
6. **No hidden condition any more, and no `hidden:` mechanism either.**
   `status = active` (§1.5, §2.12) was the only hidden condition;
   `datev_bookings` has no soft-delete status column any more, so both `HIDDEN`
   constants stood empty. They and `Compiler#apply(hidden:)` are **gone**
   (2026-09): host-pinned conditions are the policy's **fixed slots**
   (`show: :readonly` renders them read-only, `show: :hidden` not at all; both
   are parsed strictly and compiled by `state.filter.scope`) — see UX iteration 5,
   "Where the filter's own state lives" above and the superseded decision in
   §2.3.
...
- **Verified end-to-end** (desktop + 375 px mobile): build/edit/delete conditions
  ...
  presence atoms; Anwenden → PRG → canonical `?f=` URL; chips restored from
  ...
  Zurücksetzen clears it; garbage `?f=` renders the unfiltered page (no
...
- **Small usability addition beyond the doc**: when the builder state differs
  from the applied filter, a warning chip ("Änderungen noch nicht angewendet")
  and a badge on the filter toggle appear, Anwenden is highlighted, and an
  "Änderungen verwerfen" button restores the applied filter client-side. The
  reset link is labelled "Filter zurücksetzen" and applies the table's default
  filter.
...
- **Text-search sub-variants**: three attributes
  `text_any` (Buchungstext & Belege: posting_text + original_posting_text +
  Belegfeld 1 + 2, short_key `qa`; **default**), `text` (Buchungstext:
  posting_text + original_posting_text, `q`) and `text_document`
  (Belege: Belegfeld 1 + 2, `qb`), joined by the new generic attribute
  ...
  when switching. The widest variant is the default — a booking is looked up by
  whatever text it carries — and the two narrower ones restrict the search to
  the Buchungstext or to the Belegfelder.
...
- **Host-side exclusion**: `bind(base, except: […])` (and
  `Fin::DatevBookingsFilterSchema.bound(except: …)`) lets a controller hide attributes;
  the bookings page excludes **Sphäre and Beitragsbuchung**
...
- **Whole month / whole year date atoms**: new vocabulary operators
  ...
  field).
- **Variant switch is a dropdown** (Buchungstext & Belege | Buchungstext |
  Belege), default Buchungstext & Belege; the typed term survives
  switching.
- **Text operator set**: `enthält` (contains), `enthält nicht` (not_contains),
  `Glob` (glob), `ist genau` (eq) and `Regex` (regex), each in a
  ...
  substring (ILIKE/LIKE), glob = ILIKE/LIKE over the WHOLE value with `*`
  (any run) and `?` (exactly one) as the only wildcards — the term's own LIKE
  specials are escaped first, so `*RK*` is the substring form and `RK*` the
  prefix one —, regex = POSIX `~*`/`~` (Ruby-side syntax check;
  a PG-only-invalid pattern would still error — rare, accepted for now).
  Glob carries generic operator metadata **`hint:`** — a short sentence about
  the operator's operand, which the editor appends to the operand control's
  keyboard hints.
...
- **Pane redesign**: `<details>` replaced by `.pane-body` plus a toggle — a
  remote `.pane-toggle` naming its pane in `data-pane-target`, see
  `shared/wsjrdp/_collapsible_panes`. Where the filter pane's toggle sits, and
  which badges it carries, is UX iteration 7. (`.pane-pill`, the in-pane pill
  with the body to its right, is supported by the same partial for other panes.)
...
### UX iteration 5 (implemented, 2026-08) — reconciliation & shared table
...
- **Shared table widget consolidation** (`shared/wsjrdp/_expandable_table`): the
  generic detail-table gained opt-in **pagination above + below** (incl. a
  per-page select), **multi-select** (shared `shared/wsjrdp/_table_selection_js`, now
  scope-based via `.bk-select-scope` so the select-all bar can live in a
  toolbar), a right-aligned **column hamburger** (`shared/wsjrdp/_table_hamburger` +
  `shared/wsjrdp/_expandable_table_columns_form` — show/hide + drag-reorder, the
  state carried in the `<prefix>c` query param, bookmarkable, no
  localStorage), and click-to-sort headers. The
...
- **Accounting-entry detail**: a shared read-only partial
  (`fin/accounting_entries/_detail`, built on `shared/wsjrdp/_kv_grid`) mirrors the
...
### UX iteration 6 (implemented, 2026-09) — amounts by magnitude

- **Two global operators for amounts**: `lte` (`le`, "≤" on decimal, "bis" on
  date — the inclusive upper bound `between` uses, next to the exclusive
  `lt`/"vor") and `nonzero` (`nz`, "≠ 0", **arity `:none`**, decimal only —
  the operand-less "has a value at all" of a signed amount). No `zero`
  counterpart: `eq 0` already says that.
- **Accepted vs. pickable operators** (§2.2): an attribute's `operators:` is
  everything it accepts and `pickable:` names the subset the editor offers. The
  catalog flags every operator, the builder lists only the pickable ones **plus
  the edited condition's own**, and chips label anything accepted. The finance
  schemas declare no narrowing: on every amount, count and balance the editor
  offers the whole accepted list, and its ORDER is the declaration's.
- **Sign pairs — the second use of `variant_group:`.** An amount and its
  magnitude are ONE picker entry, the two members marked `sign: :signed` and
  `sign: :absolute`:

  | dataset | picker entry | signed | absolute |
  |---|---|---|---|
  | Buchungen | Betrag | `amount` (`amt`) | `amount_abs` (`amta`) |
  | Buchungen | Betrag (Original-Währung) | `amount_original` (`amo`) | `amount_original_abs` (`amoa`) |
  | Kreditoren | Saldo | `booking_balance` (`bb`) | `booking_balance_abs` (`bba`) |
  | Moss-Transaktionen | Betrag (Transaktion) | `amount` (`amt`) | `amount_abs` (`amta`) |
  | Moss-Transaktionen | Betrag (alle Ebenen) | `amount_any` (`ama`) | `amount_any_abs` (`amaa`) |
  | Moss-Wallet | Betrag | `amount` (`amt`) | `amount_abs` (`amta`) |

  The absolute member is an `ABS(...)` column (an Arel `NamedFunction` in the
  attribute's `column:` lambda) — for "Betrag (alle Ebenen)" all three of them,
  transaction, expense and booking, each wrapped and the comparison an OR
  across them. So "|Betrag| ≥ 1000" is one atom instead of an OR over both
  signs. **Which member is the default follows the data** (the first declared
  one wins): the Kreditoren start on |Saldo| — a creditor's balance is negative
  as often as positive — everywhere else the signed member leads.
- **The editor renders a sign pair as a toggle**, not as the sub-variant
  dropdown the text searches use: two segmented buttons, `±` and `|x|`, the
  edited member pressed (`.flt-sign`, §2.5). `Schema.define` checks the pairing
  at declaration time — a group either declares no `sign` at all, or has
  exactly two members, one `:signed` and one `:absolute`.
- **Operator lists per numeric field**, first entry = the operator a fresh
  condition starts on:
  - Buchungen `Betrag`, both Moss `Betrag`s and the wallet `Betrag`:
    `im Bereich`, `≤`, `≥`, `<`, `>`, `=` — a range is what an amount is
    usually asked for.
  - Kreditoren `Saldo` and `Buchungen` (the count): `≠ 0`, `im Bereich`, `≤`,
    `≥`, `<`, `>`, `=` — "has a balance / has bookings at all" comes first.
    The count carries no sign toggle; it has no negative side.
  - `Fin::MossTransactionsFilterSchema::AMOUNT_ANY`, the multi-column decimal
    type behind "Betrag (alle Ebenen)", implements the same six comparisons
    with the same labels as `Types::DECIMAL`, each ORing across its columns.
- **`operand_min:`** is generic attribute metadata: an absolute member declares
  `operand_min: 0` and the editor's number input gets `min="0"`. It is guidance
  only — the server does not re-validate the sign, because a negative bound on
  an `ABS()` column is harmless (it simply matches everything or nothing).
- **The "Nur mit Saldo ≠ 0" preset is one condition**,
  `[["booking_balance_abs", "nonzero"]]` (wire `!(!(!(bba,nz)))`) — the preset
  that motivated the whole iteration. Its twin `Saldo ≠ 0` counts as the same
  condition: a sign pair is one attribute under a **sign-invariant** operator
  (`≠ 0`, `hat Wert`, `ist leer`, `= 0` — none of them asks about the sign), so
  the preset is pressed on either member and its toggle removes the one the
  user has, while a comparison stays member-specific. `Schema#sign_aliases`
  ships the pairing to `Wsjrdp::Filtering::SlotEquality`; the chips derive the
  same map from the catalog, the builder's JS from `variant_group`/`sign`.
  "Nur mit Buchungen" is `[["booking_count", "nonzero"]]` for the same reason:
  for a count `≠ 0` and `> 0` are the same predicate, and `≠ 0` is what the
  editor offers first, so a hand-built `Buchungen ≠ 0` is recognised.

### UX iteration 7 (implemented, 2026-09) — the filter line

- **The filter line** (`shared/wsjrdp/filtering/_line`): one framed strip
  directly above the table's toolbar, rendered whether the pane is open or
  closed. Left, in `.flt-line-left`, the "Schnellauswahl" preset bar (DOM id
  `et_filter_presets_id(state)`; it sits outside `.flt-root`, which is why the
  builder's `lockPresets()` reaches it by that id) and, after it, the applied
  filter's chips. Right, at the end of the line, the pane's toggle
  (`.pane-toggle[data-pane-target]`) with the count of applied user conditions —
  a preset's own slot included — as a badge and the "nicht angewendet" pill. The
  column hamburger belongs to the toolbar, not to this line.
- **The pane opens between the line and the toolbar** and is `display: none`
  while closed, so opening it moves nothing above it: the toggle stays under the
  pointer that clicked it. The line carries `data-pane-line` with the pane's id,
  and `shared/wsjrdp/_collapsible_panes` mirrors the pane's open state onto it,
  so line and open pane read as one box. The open state is the table's `pane`
  field (`e`, a cookie), resolved by the controller.
- **The applied filter as chips** (`shared/wsjrdp/filtering/_chips`, worded by
  `Wsjrdp::FilterChips` — pure value logic over the catalog and the applied
  tree, mirroring the builder's `condFullText`): one chip per slot of
  `state.filter.user_slots`, in slot order, with "und" between them, because the
  slots are ANDed. A chip is a `.pane-opener` button: it only ever OPENS the
  pane, and it carries no remove "×" — removing a condition is an edit, and
  edits happen in the builder. While the pane is open the chips hide; the
  builder below shows the same conditions in full.
- **What gets no chip**: a slot equal to an ACTIVE preset's (the toggle to its
  left already shows that state, by the same `SlotEquality` rule that decided
  `active?`), a slot of a preset group whose `in` values are all member values
  (the pressed member buttons show it), and every fixed slot (`user_slots` is
  the user half alone; the `readonly` ones are the builder's locked chips).
  With no chip at all the line
  reads "Kein Filter aktiv" only when the table has neither presets nor fixed
  slots — a table with presets shows the bar instead, a table whose conditions
  are all pinned shows nothing.
- **Specs**: `spec/domain/wsjrdp/filter_chips_spec.rb` (standalone: the wording
  and the preset rule) and the line's markup in the host controller specs of the
  Kreditoren, the Moss wallet and the Moss transactions.
