...
- **Never relax these.** `&`, `=`, `+`/`%2B`, `#`, `%` (always `%25`), and
  ...
  gen-delims and encode Rack's param nesting (`show[all]=1`).
...
- **Safe default — when unsure, do not relax.** An over-encoded URL is always
  ...
  `Wsjrdp::RelaxedUrlQuery.to_query` / `wsjrdpRelaxedQuery`, emit the fully encoded form
...
- Used in the code: [`Hash#to_query`][to-query] is what `Wsjrdp::RelaxedUrlQuery` (§6) builds on —
...
4. **Do not scatter** raw `.gsub("%2C", ",")` calls. If you need this behaviour in
   ...
   inline expression.
5. **This is enforced, not just asked.** `spec/domain/wsjrdp/url_encoding_guard_spec.rb`
   ...
   change, call `Wsjrdp::RelaxedUrlQuery` / `wsjrdpRelaxedQuery` instead. Extending the
...
- **Ruby** — [`Wsjrdp::RelaxedUrlQuery`](../app/domain/wsjrdp/relaxed_url_query.rb)
  (`app/domain/wsjrdp/relaxed_url_query.rb`): the only public API is
  `Wsjrdp::RelaxedUrlQuery.to_query(params_hash)` → query string with `,`/`~` literal.
  ...
  ```ruby
  "#{request.path}?#{Wsjrdp::RelaxedUrlQuery.to_query(query_params)}"
  ```

  Tests: `spec/domain/wsjrdp/relaxed_url_query_spec.rb` — readability, round-trips
...
- **JavaScript** — `window.wsjrdpRelaxedQuery`
  ([`app/views/shared/wsjrdp/_relaxed_query_js.html.haml`](../app/views/shared/wsjrdp/_relaxed_query_js.html.haml)):
  ...
  the partial (`= render "shared/wsjrdp/relaxed_query_js"`, idempotent per
...
`Wsjrdp::RelaxedUrlQuery.to_query` when you touch it; never "fix" it with an inline
