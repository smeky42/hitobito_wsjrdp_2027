...
The Wagon has a semantic-versioning-like version number
...
## Source of the Version Number in the Running App
...
* **First line**: `vX.Y.Z <summary>` — the new version number plus a
  ...
  becomes the tag message and is what humans read in `git tag -n` and
  in the deploy channel.
* **Body**: a markdown bullet list (`*`), one bullet per commit since
  ...
  bullet with the commit's GitHub PR in parentheses,
  e.g. `(#123)`. Collect the raw list with:
...
2. Pick the new number (see Numbering) after reviewing
   `git log v<PREVIOUS>..HEAD --oneline`.
3. Edit `lib/hitobito_wsjrdp_2027/version.rb`, stage it — nothing else
