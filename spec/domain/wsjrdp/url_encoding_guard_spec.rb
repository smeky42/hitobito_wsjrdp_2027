...
# Enforces the encapsulation rule of doc/wsjrdp/url_encoding.md §6: the %XX -> literal
...
#   bundle exec rspec ../hitobito_wsjrdp_2027/spec/domain/wsjrdp/url_encoding_guard_spec.rb
require "pathname"

describe "URL-encoding guard (doc/wsjrdp/url_encoding.md §6)" do
  ...
  # change -- see doc/wsjrdp/url_encoding.md §6.1.
  let(:allowed) do
    ["app/domain/wsjrdp/relaxed_url_query.rb",
  ...
  it "finds no %XX relax outside the two mandatory helpers" do
    ...
    expect(offenders).to be_empty, <<~MSG
      ...
      This is forbidden by doc/wsjrdp/url_encoding.md §6: relaxing %XX is safe ONLY on
      the immediate output of a percent-encoder. Use Wsjrdp::RelaxedUrlQuery.to_query
      (Ruby) or window.wsjrdpRelaxedQuery (JS) instead. Adding a file to ALLOWED
      is itself a policy change -- see doc/wsjrdp/url_encoding.md §6.1.
    MSG
  end
