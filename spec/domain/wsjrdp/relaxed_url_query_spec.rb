...
# Deliberately standalone (no rails spec_helper): Wsjrdp::RelaxedUrlQuery is pure, the
...
#
#   bundle exec rspec ../hitobito_wsjrdp_2027/spec/domain/wsjrdp/relaxed_url_query_spec.rb
require "active_support"
require "active_support/core_ext/object/to_query"
require "rack/utils"
# The file declares `module Wsjrdp::RelaxedUrlQuery` (compact style, like every
# other kit PORO). Loading it directly -- without Rails/Zeitwerk -- therefore
# needs the parent constant to exist first.
module Wsjrdp; end
require_relative "../../../app/domain/wsjrdp/relaxed_url_query"

# Round-trip safety of the relaxed query encoding (doc/wsjrdp/url_encoding.md §6):
...
describe Wsjrdp::RelaxedUrlQuery do
  def roundtrip(params)
    Rack::Utils.parse_nested_query(Wsjrdp::RelaxedUrlQuery.to_query(params))
  end
  ...
  describe "readability (the point of the relaxing)" do
    it "keeps , and ~ literal in values" do
      expect(Wsjrdp::RelaxedUrlQuery.to_query(sort: "bez,nr~")).to eq("sort=bez,nr~")
    end

    it "keeps , and ~ literal across several params (A-Rison sort + cols)" do
      query = Wsjrdp::RelaxedUrlQuery.to_query("sort" => "bez~,cnt~", "cols" => "nr,bez,~st,sum,cnt")
    ...
    it "returns an empty string for empty params" do
      expect(Wsjrdp::RelaxedUrlQuery.to_query({})).to eq("")
    end
  end
  ...
  describe "encapsulation (no raw-string relax API)" do
    it "exposes only to_query -- no public method accepts a pre-built string" do
      expect(Wsjrdp::RelaxedUrlQuery.singleton_methods(false)).to contain_exactly(:to_query)
    end

    it "only relaxes characters that are provably separator-free" do
      expect(Wsjrdp::RelaxedUrlQuery.const_get(:RELAXATIONS).values).to all(match(/\A[,~]\z/))
    end

    # The relax pieces must not be reachable from outside: otherwise
    # `str.gsub(Wsjrdp::RelaxedUrlQuery::RELAXATION_PATTERN, Wsjrdp::RelaxedUrlQuery::RELAXATIONS)`
    ...
    [:RELAXATIONS, :RELAXATION_PATTERN].each do |const|
      it "keeps #{const} private" do
        expect(Wsjrdp::RelaxedUrlQuery.const_get(const)).not_to be_nil # it exists ...
        ...
        expect { eval("Wsjrdp::RelaxedUrlQuery::#{const}", binding, __FILE__, __LINE__) } # standard:disable Security/Eval
          .to raise_error(NameError, /private constant/)
      end
    end

    it "exposes no public constants at all" do
      expect(Wsjrdp::RelaxedUrlQuery.constants(false)).to be_empty
    end
    ...
    ["sort=bez%2Cnr", "", "%2C"].each do |bad|
      it "raises TypeError for the pre-built string #{bad.inspect}" do
        expect { Wsjrdp::RelaxedUrlQuery.to_query(bad) }.to raise_error(TypeError, /expects a Hash/)
      end
    end
