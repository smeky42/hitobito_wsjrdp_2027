...
# signed_offsetting_base_amount column (see doc/bookkeeping.md). The three bookings below are
...
describe DatevBooking do
  def booking(absolute_amount, attrs)
    DatevBooking.create!({buchungs_guid: SecureRandom.uuid,
                          ...
                          base_amount: absolute_amount,
                          transaction_amount: absolute_amount}.merge(attrs))
  end

  # expense (Soll -> D) -> creditor (Haben): we owe 660 (amount: -660)
  let!(:invoice) do
    booking(660, account_number: "66600", account_kind: "EXPENSE",
      offsetting_account_number: "700017", offsetting_account_kind: "CREDITOR",
      debit_credit: "D")
  end
  # clearing (Haben -> C) -> creditor (Soll): settles the 660 (amount: -660)
  let!(:payment) do
    booking(660, account_number: "36100", account_kind: "CLEARING",
      offsetting_account_number: "700017", offsetting_account_kind: "CREDITOR",
      debit_credit: "C")
  end
  # bank (Soll -> D) -> income (Haben): +660 income (income sits on the Gegenkonto)
  let!(:fee) do
    booking(660, account_number: "18000", account_kind: "BANK",
      offsetting_account_number: "41030", offsetting_account_kind: "INCOME",
      debit_credit: "D")
  end

  def balance(account)
    DatevBooking.legs.where(leg_account_number: account).sum(:signed_leg_amount)
  end

  describe "generated amount" do
    it "is the booking value from the Konto's perspective" do
      expect(invoice.reload.signed_base_amount).to eq(-660) # expense: money out
      expect(payment.reload.signed_base_amount).to eq(-660) # clearing: money out
      expect(fee.reload.signed_base_amount).to eq(660)      # bank: money in
    end
  end

  describe "generated signed_offsetting_base_amount" do
    it "is the booking value from the Gegenkonto's perspective" do
      expect(invoice.reload.signed_offsetting_base_amount).to eq(-660) # creditor: we owe
      expect(payment.reload.signed_offsetting_base_amount).to eq(660)  # creditor: settled
      expect(fee.reload.signed_offsetting_base_amount).to eq(660)      # income: +660
    end
  end
  ...
  describe ".legs" do
    ...
    it "keeps a bank account (always the Konto) equal to SUM(signed_base_amount)" do
      expect(balance("18000")).to eq(DatevBooking.where(account_number: "18000").sum(:signed_base_amount))
    end

    it "keeps the trial balance (account-centric Soll+/Haben-) at exactly zero" do
      account_centric = DatevBooking.legs.sum(
        Arel.sql("signed_leg_amount * (CASE WHEN leg_account_kind IN ('INCOME','EXPENSE') THEN -1 ELSE 1 END)")
      )
