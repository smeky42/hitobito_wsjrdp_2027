...
# (doc/fin/bookkeeping_schema_review.md §4): *_ref_type is STORED GENERATED from
...
describe DatevBooking do
  ...
  let!(:bank) { WsjrdpLedgerAccount.create!(number: "18000", name: "PAX", account_kind: "BANK") }
  let!(:expense) { WsjrdpLedgerAccount.create!(number: "66500", name: "Reise", account_kind: "EXPENSE") }
  let!(:supplier) { WsjrdpPersonalAccount.create!(number: "700019", name: "Förderkreis") }

  it "derives the ref types from the Kontenart" do
    b = booking(offsetting_account_number: "700019", offsetting_account_kind: "CREDITOR")
    expect(b.reload.account_type).to eq("WsjrdpLedgerAccount")
    expect(b.offsetting_account_type).to eq("WsjrdpPersonalAccount")
  end

  it "navigates to the ledger account and the supplier" do
    b = booking(offsetting_account_number: "700019", offsetting_account_kind: "CREDITOR").reload
    expect(b.account).to eq(bank)
    expect(b.offsetting_account).to eq(supplier)
  end

  it "keeps account_kind as the Kontenart, not as the association type" do
    expect(booking.reload.account_kind).to eq("BANK")
  end

  it "builds where(offsetting_account: record) on ref_type + number" do
    hit = booking(offsetting_account_number: "700019", offsetting_account_kind: "CREDITOR")
    booking # non-matching
    sql = described_class.where(offsetting_account: supplier).to_sql
    expect(sql).to include(%("offsetting_account_type" = 'WsjrdpPersonalAccount'))
  ...
  it "preloads mixed targets without N+1" do
    booking
    booking(offsetting_account_number: "700019", offsetting_account_kind: "CREDITOR")
  ...
  it "returns nil (not an error) for numbers missing from the reference tables" do
    b = booking(account_number: "99999", account_kind: "UNKNOWN").reload
