...
module WsjrdpFinanceCapHelper
  ...
  # without any finance permission (and for abilities that carry no user
  # context, e.g. a service token's).
  def finance_tiers
    ...
    return nil if by_roles.nil?

    {by_roles: by_roles, effective: context.finance_tier}
  end

  # Does the cap actually lower the tier? No cap, a cap at or above the roles'
  # tier, or no finance permission at all: no.
