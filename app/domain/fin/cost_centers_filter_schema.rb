# frozen_string_literal: true

#  Copyright (c) 2026 German Contingent for the World Scout Jamboree 2027.
#
#  This file is part of hitobito_wsjrdp_2027 and licensed under the
#  Affero General Public License version 3 or later. See the COPYING
#  file at the top-level directory or at
#  https://github.com/smeky42/hitobito_wsjrdp_2027

# The Kostenstellen dataset for the generic CNF filter
# (doc/wsjrdp/generic_filter_builder.md): attribute declarations and the
# request-time binding, one consumer of the filter-schema protocol next to
# Fin::DatevBookingsFilterSchema, Fin::MossTransactionsFilterSchema and
# Fin::PersonalAccountsFilterSchema.
#
# The base relation is WsjrdpCostCenter.with_booking_summary, so `Buchungen` is
# an ordinary column the compiler can filter on -- the page never filters an
# Array in Ruby.
module Fin::CostCentersFilterSchema
  extend Wsjrdp::Filtering::FilterSchema

  # Moss knows a cost center as active or deactivated; one it does not know at
  # all has moss_status NULL. NULL counts as INAKTIV here (the column's own
  # comment says the same), which is why the attribute filters on
  # COALESCE(moss_status, 'deactivated') rather than on the raw column -- the
  # same expression the table's "Moss Status" cell and its sort use.
  MOSS_STATUS_OPTIONS = Wsjrdp::Filtering::Options.values(-> {
    [[WsjrdpCostCenter::STATUS_ACTIVE, I18n.t("fin.moss_status.active")],
      [WsjrdpCostCenter::STATUS_DEACTIVATED, I18n.t("fin.moss_status.deactivated")]]
  })

  # All case variants of the text operators, the same list the Buchungen page
  # offers (Fin::DatevBookingsFilterSchema::TEXT_OPERATORS), glob pair included:
  # a cost-center name is exactly the kind of value one wants to match with
  # `Unit *` or `*Lager`. The UI collapses each ci/cs pair into one entry plus
  # the "Groß-/Kleinschreibung" toggle.
  TEXT_OPERATORS = %i[
    contains contains_cs not_contains not_contains_cs glob glob_cs eq eq_cs regex regex_cs
  ].freeze

  # What the Buchungen count accepts and offers, in the editor's order. `≠ 0` is
  # what the "Nur mit Buchungen" preset is made of and the first entry a user
  # sees, so a fresh condition starts on "has bookings at all"; the comparisons
  # follow.
  COUNT_OPERATORS = %i[nonzero between lte gte lt gt eq].freeze

  # Declaration order = picker order. The long keys are the COLUMN keys of
  # Fin::BookkeepingSummaryColumns::COST_CENTERS, so a condition and a sorted
  # header speak of the same thing.
  SCHEMA = Wsjrdp::Filtering::Schema.define do |s|
    # One text search over both name columns (the multi-column form of the text
    # type: a positive predicate matches in EITHER, "enthält nicht" in neither),
    # so a search for a Kurzbezeichnung finds its cost center as readily as a
    # search for the long Bezeichnung does.
    s.attribute key: :name, short_key: :q, label: "Name",
      type: Wsjrdp::Filtering::Types::TEXT, operators: TEXT_OPERATORS,
      column: ->(t) { [t[:name], t[:short_name]] }
    s.attribute key: :moss_status, short_key: :ms, label: "Moss Status",
      type: Wsjrdp::Filtering::Types::ENUM, operators: %i[in not_in],
      column: ->(t) {
        Arel::Nodes::NamedFunction.new("COALESCE",
          [t[:moss_status], Arel::Nodes.build_quoted(WsjrdpCostCenter::STATUS_DEACTIVATED)])
      },
      options: MOSS_STATUS_OPTIONS
    # A count, but the decimal type is the right one: its operand cast
    # (BigDecimal) compares exactly against an integer column, and the UI's
    # number input is what a count wants anyway. No sign toggle -- a count has
    # no negative side.
    s.attribute key: :booking_count, short_key: :bc, label: "Buchungen",
      type: Wsjrdp::Filtering::Types::DECIMAL, operators: COUNT_OPERATORS,
      column: :booking_count
    # NOT in the picker (catalog: false): the number exists so the page can PIN
    # a cost center out of its list as a hidden fixed slot
    # (Fin::CostCentersController::HIDDEN_COST_CENTER_NUMBER). A user filters by
    # name, not by number.
    s.attribute key: :number, short_key: :nr, label: "Kostenstelle",
      type: Wsjrdp::Filtering::Types::REFERENCE, operators: %i[in not_in],
      catalog: false, column: :number
  end

  # `except:` lets the host page hide attributes (the table's `exclude:`). The
  # base carries the booking summary, so every relation the compiler runs over
  # has the two aggregate columns -- Wsjrdp::Filtering::FilterSchema#compile
  # merges this base into whatever relation a host hands in.
  def self.bound(except: nil)
    SCHEMA.bind(WsjrdpCostCenter.with_booking_summary, except: except)
  end
end
