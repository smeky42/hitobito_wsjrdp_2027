...
# (accounting entry). See doc/fin/bookkeeping_schema_review.md §3c.
#
...
#
# connect!/connect_pair! set accounting_entries.datev_booking_id + the link
# provenance in ONE bulk UPDATE and mirror the entry's bank transaction onto
# wsjrdp_camt_transactions.datev_booking_id.
module Fin::DatevBookingMatcher
  ...
  # user-adjustable, see doc/fin/recon_linking.md): a computed rating STRICTLY GREATER
  ...
  def self.propose(bookings)
    ...
    linked = AccountingEntry.where(datev_booking_id: bookings.map(&:id))
      .pluck(:datev_booking_id).to_set
    bookings = bookings.reject { |b| linked.include?(b.id) }
  ...
  def self.propose_for_entries(entries)
    # Entries now carry datev_booking_id directly -- drop the linked ones with
    # a plain column read, no extra query.
    entries = entries.to_a.reject(&:datev_booking_id)
  ...
  # `signed_offsetting_base_amount` from the Gegenkonto perspective. Empirically this
  ...
  def self.signed_cents(booking)
    amount = if booking.account_number == FEE_ACCOUNT
      ...
      booking.signed_offsetting_base_amount || -booking.signed_base_amount
    end
    (amount * 100).round
  end

  def self.connect_pair!(booking, entry, linked_by_id: nil)
    return false if entry.datev_booking_id ||
      AccountingEntry.exists?(datev_booking_id: booking.id)
  ...
  class << self
    ...
    # classification_string already marks it import-equivalent: score 100,
    ...
    def rate_scored_pair(booking, entry)
    ...
    # pairs actually connected; the entry's bank transaction is mirrored onto the
    # camt side afterwards. Returns the number of pairs connected.
    def write_pairs(pairs, linked_by_id: nil)
      connection = AccountingEntry.connection
      now = Time.zone.now
      # detect_link_type / link_score need the booking object; callers pass ids.
      bookings = DatevBooking.where(id: pairs.map(&:first)).index_by(&:id)
      rows = pairs.map { |booking_id, entry|
        booking = bookings[booking_id]
        classification = detect_link_type(booking, entry)
        meta = link_meta(now: now, author_id: linked_by_id,
          score: link_score(booking, entry, classification),
          automatic_manual: "manual", classification: classification)
        tuple = [entry.id, booking_id, meta.to_json]
        "(#{tuple.map { |v| connection.quote(v) }.join(", ")})"
      }.join(", ")
      linked = connection.execute(<<~SQL).cmd_tuples
        UPDATE accounting_entries AS ae SET
          datev_booking_id = v.booking_id::bigint,
          datev_booking_link_meta = v.meta::jsonb,
          updated_at = #{connection.quote(now)}
        FROM (VALUES #{rows}) AS v(entry_id, booking_id, meta)
        WHERE ae.id = v.entry_id::bigint
          AND ae.datev_booking_id IS NULL
          AND NOT EXISTS (SELECT 1 FROM accounting_entries other
                          WHERE other.datev_booking_id = v.booking_id::bigint)
      SQL
      mirror_camt_links(pairs.map { |_booking_id, entry| entry.id }, now) if linked.positive?
      linked
    end

    # The datev_booking_link_meta JSON object (see the migration for the keys).
    def link_meta(now:, author_id:, score:, automatic_manual:, classification:)
      {
    ...
    def link_score(booking, entry, classification)
      ...
      match = rate_scored_pair(booking, entry)
      match && (match.score / 100.0)
    end

    # Mirror each just-linked entry's bank transaction onto the camt side
    # (wsjrdp_camt_transactions.datev_booking_id) -- the reversed equivalent of
    # the DATEV importer's camt mirror. Only entries actually linked (datev_booking_id
    # set) are considered.
    def mirror_camt_links(entry_ids, now)
      return if entry_ids.empty?

      connection = AccountingEntry.connection
      ids = entry_ids.map { |id| connection.quote(id) }.join(", ")
      connection.execute(<<~SQL)
        UPDATE wsjrdp_camt_transactions AS c SET
          datev_booking_id = ae.datev_booking_id, updated_at = #{connection.quote(now)}
        FROM accounting_entries ae
        WHERE ae.id IN (#{ids})
          AND ae.datev_booking_id IS NOT NULL
          AND ae.camt_transaction_id = c.id
    ...
    def booking_context(booking)
      ...
      text_norm = normalize(booking.posting_text.presence || raw_text)
      {
        text_norm: text_norm,
        ...
        codes: extract_codes([booking.posting_text, raw_text,
    ...
    def score_pair(booking, entry, names, context, entry_codes)
      name = names[entry.subject_id]
      return nil unless name

      person_score, person_reason = person_component(entry, name, context)
    ...
    def person_component(entry, name, context)
    ...
    def propose_einzug_for_entries(entries, proposals, used_booking_ids)
      ...
      bookings = DatevBooking.without_accounting_entry
    ...
    def propose_scored_for_entries(entries, proposals, alternatives, used_booking_ids)
      ...
      pool = DatevBooking.without_accounting_entry.where(Arel.sql(fee))
      ...
      if all_codes.any?
        pattern = all_codes.map { |c| Regexp.escape(c) }.join("|")
        code_pool = DatevBooking.without_accounting_entry.where(Arel.sql(fee))
          .where("document_field_1 ~* :p OR document_field_2 ~* :p OR posting_text ~* :p OR original_posting_text ~* :p",
            p: "(#{pattern})").to_a
    ...
    def build_reverse_match(entry, scored)
      ...
      details = [
        ...
        "Person: #{scored[:name][:display]}",
        "Buchung #{I18n.l(booking.booking_date)} · #{booking.posting_text.to_s.truncate(60)}",
        "Person-Match: #{scored[:person_reason]} (#{scored[:person_score]} %)",
        "Datums-Match: #{scored[:date_reason]} (#{scored[:date_score]} %)",
        "Score: #{scored[:person_score]} % × #{scored[:date_score]} % = #{scored[:score]} %"
      ]
