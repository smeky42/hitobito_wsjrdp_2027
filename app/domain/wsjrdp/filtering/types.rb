...
module Wsjrdp::Filtering
  ...
  module Types
    ...
    # Glob: `*` (any run of characters) and `?` (exactly one) are the ONLY
    # wildcards; LIKE's own specials in the user's text are escaped first, so
    # they stay literal. The pattern is matched against the WHOLE value -- no
    # implicit %...% -- which is what makes it a glob: `*RK*` is the substring
    # form, `RK*` the prefix one.
    GLOB_WILDCARDS = {"*" => "%", "?" => "_"}.freeze
    GLOB_HINT = "* steht für beliebig viele Zeichen, ? für genau eines; " \
                "das Muster muss den ganzen Text treffen."

    def self.glob_predicate(cols, term, case_sensitive)
      pattern = escape_like(term).gsub(/[*?]/, GLOB_WILDCARDS)
      wrap(cols).map { |c| c.matches(pattern, nil, case_sensitive) }.reduce(:or)
    end
    ...
    # `nonzero` is the operand-less "has any value at all" of a signed amount:
    # NULL stays UNKNOWN (3VL, §1.3), so a row without a value is excluded just
    # as it is by every comparison.
    DECIMAL = Type.new(key: :decimal, control: "number_range", operators: [
      Operator.new(key: :eq, label: "=", arity: :one) { |c, v| c.eq(v[0]) },
      Operator.new(key: :gte, label: "≥", arity: :one) { |c, v| c.gteq(v[0]) },
      Operator.new(key: :gt, label: ">", arity: :one) { |c, v| c.gt(v[0]) },
      Operator.new(key: :lt, label: "<", arity: :one) { |c, v| c.lt(v[0]) },
      Operator.new(key: :lte, label: "≤", arity: :one) { |c, v| c.lteq(v[0]) },
      Operator.new(key: :between, label: "im Bereich", arity: :two) { |c, v| c.gteq(v[0]).and(c.lteq(v[1])) },
      Operator.new(key: :nonzero, label: "≠ 0", arity: :none) { |c, _| c.not_eq(0) },
    ...
    # whole-month / whole-year atoms with their own operand controls. `lte`
    # ("bis") is the inclusive upper bound -- the end `between` already uses --
    # next to the exclusive `lt` ("vor").
    DATE = Type.new(key: :date, control: "date_range", operators: [
      ...
      Operator.new(key: :lte, label: "bis", arity: :one) { |c, v| c.lteq(v[0]) },
      Operator.new(key: :between, label: "im Bereich", arity: :two) { |c, v| c.gteq(v[0]).and(c.lteq(v[1])) },
    ...
    TEXT = Type.new(key: :text, control: "text", operators: [
      ...
      Operator.new(key: :glob, label: "Glob", arity: :one, hint: GLOB_HINT,
        case_group: :glob, case_sensitive: false) { |cols, v| Types.glob_predicate(cols, v[0], false) },
      Operator.new(key: :glob_cs, label: "Glob (Groß/Klein)", arity: :one, hint: GLOB_HINT,
        case_group: :glob, case_sensitive: true) { |cols, v| Types.glob_predicate(cols, v[0], true) },
