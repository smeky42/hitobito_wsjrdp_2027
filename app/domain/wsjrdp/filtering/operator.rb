...
module Wsjrdp::Filtering
  ...
  # hint: short editor hint about the operator itself (what its operand means),
  #   appended to the operand control's keyboard hints.
  ...
  #   regex validation).
  class Operator
    attr_reader :key, :short_key, :label, :arity, :label_many, :many_hint, :hint,
      :case_group, :case_sensitive, :operand_control, :cast

    def initialize(key:, label:, arity:, label_many: nil, many_hint: nil, hint: nil,
    ...
    def as_json(*)
      {key: key, label: label, arity: arity, label_many: label_many,
       many_hint: many_hint, hint: hint, case_group: case_group,
