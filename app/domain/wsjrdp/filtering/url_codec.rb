...
module Wsjrdp::Filtering
  ...
  module UrlCodec
    ...
    # Percent-encode characters that would break URL/query parsing (&, +, #,
    # %, whitespace, ;, ?) and non-ASCII bytes (umlauts etc.); everything else
    # stays readable.  The result is placed directly as the query value --
    # never additionally form-encoded.
    def escape_for_query(rison)
      rison.gsub(/[%&+#;?\s]|[^\x00-\x7F]/) { |c|
