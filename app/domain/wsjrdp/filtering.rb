...
# Generic CNF filter engine (doc/wsjrdp/generic_filter_builder.md). A filter is an AND
...
# app/domain/wsjrdp/filtering/.
#
# The engine is dataset-agnostic: datasets declare a Wsjrdp::Filtering::Schema
...
module Wsjrdp::Filtering
  ...
  OPERATOR_VOCABULARY = {
    ...
    lte: :le,                # <=  (decimal, date; date label "bis")
    between: :bt,            # closed range [a, b]
    nonzero: :nz,            # <> 0 ("≠ 0"), no operand -- decimal only
    ...
    glob: :gl,               # whole-value ILIKE with * / ? wildcards ("Glob")
    glob_cs: :glc,           # whole-value LIKE with * / ? wildcards
